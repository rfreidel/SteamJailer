...
void install_wine_proton() {
    ...
    run_command("sudo jexec steamjailer pkg install -y wine-proton winetricks");
    run_command("sudo jexec steamjailer /usr/local/wine-proton/bin/pkg32.sh install wine mesa-dri");
    run_command("sudo jexec steamjailer WINE=/usr/local/wine-proton/bin/wine winetricks --force corefonts dxvk nvapi vkd3d");
...
void launch_game(const std::string &game_path) {
    std::string command = "sudo jexec steamjailer WINE=/usr/local/wine-proton/bin/wine \"" + game_path + "\"";
...
void launch_steam() {
    ...
    run_command("sudo jexec steamjailer WINE=/usr/local/wine-proton/bin/wine ~/.wine/drive_c/Program\\ Files\\ \\(x86\\)/Steam/Steam.exe -cef-disable-sandbox -cef-disable-gpu-compositing -cef-in-process-gpu");
}
...
int main() {
    ...
    std::string choice = "zenity --list --title=\"Steam Game Launcher\" --column=\"Task\" Install_Wine_Proton Install_dxvk Launch_Game Launch_Steam Link_to_Home_Bin";
    char buffer[128];
    std::string result = "";
